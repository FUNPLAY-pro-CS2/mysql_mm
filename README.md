...
SQLMM will expose an interface in `OnMetamodQuery` which can then be queried with `(IMySQLInterface*)g_SMAPI->MetaFactory(MYSQLMM_INTERFACE, &ret, NULL);` by other plugins.
Example:
```c++
  IMySQLiteClient *g_pSQLiteClient;
	g_pSQLiteClient = ((IMySQLInterface *)g_SMAPI->MetaFactory(MYSQLMM_INTERFACE, nullptr, nullptr))->GetSQLiteClient();
